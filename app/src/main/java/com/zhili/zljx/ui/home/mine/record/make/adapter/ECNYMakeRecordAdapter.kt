 ...
 *2023/4/13 9:57
 */
class ECNYMakeRecordAdapter(items: List<ECNYMakeRecordBean.Data>) :
    BaseQuickAdapter<ECNYMakeRecordBean.Data, ECNYMakeRecordAdapter.VH>(items) {
    // 自定义ViewHolder类
    class VH(
        parent: ViewGroup,
    ...
    override fun onBindViewHolder(holder: VH, position: Int, item: ECNYMakeRecordBean.Data?) {
        ...
        holder.binding.apply {
            item?.let { item ->
                ...
                //预约城市
                keyValue3.setKeyTvText("预约城市")
                keyValue3.setValueTvText(item.CountyName)
                //真实姓名
                keyValue4.setKeyTvText("真实姓名")
                keyValue4.setValueTvText(item.RealName)
                //身份证号码
                keyValue5.setKeyTvText("身份证号码")
                keyValue5.setValueTvText(item.IDCard.safeIdCard())
                //手机号码
                keyValue6.setKeyTvText("手机号码")
                keyValue6.setValueTvText(item.Mobile.safePhone())
                //中签信息
//                if (item.Status == 2) {
